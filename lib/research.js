...
import { scrape, fetchable, firecrawlReady } from './firecrawl.js';
  ...
  input_schema: {
    ...
    properties: {
      questions: {
        type: 'array',
        maxItems: MAX_Q,
        description: 'Up to six questions, researched together.',
        items: {
          type: 'object',
          properties: {
            q: {
              type: 'string',
              description: 'The question, as a full sentence. Include the place and the dates where they matter — the desk cannot see the conversation.',
            },
            about: {
              type: 'string',
              description: 'Two or three words naming what this is about: "budget hotels", "getting there from the airport", "weather in September", "photo spots". The traveller reads these while they wait, so they are labels, not sentences — never repeat the question here.',
            },
            url: {
              type: 'string',
              description: 'OPTIONAL, and the difference between a real answer and a vague one. When you already know the page that holds the answer — the hotel\'s own site, the ticket page, the official opening hours, a booking listing you found earlier — put it here and the desk reads THAT page instead of searching. Use it whenever a search gave you a range off a blog and the real number is on the venue\'s own site. https only.',
            },
          },
          required: ['q'],
        },
 ...
 */
async function askOne(q, budget, model, page) {
  const body = {
    messages: [
      { role: 'system', content: WORKER_SYSTEM },
      page
        // The page is given to the WORKER, never to the chat model. That is the
        // whole discipline this desk exists for: web content in the
        // conversation is what pushed one request to 454,870 tokens.
        ? {
          role: 'user',
          content: 'Answer this from the page below. It is the source they asked for, so '
            + 'prefer what it says over anything you remember, and if it does not answer '
            + 'the question say so plainly rather than filling the gap.\n\nQUESTION: '
            + q + '\n\n--- PAGE ---\n' + page,
        }
        : { role: 'user', content: q },
    ],
  ...
  // A page we were handed needs no search, and skipping it saves the ~$0.007
  // search fee that is most of what a research round costs.
  if (page) {
...
export async function research(questions, { model } = {}) {
  // Both shapes: {q, about} is what the tool asks for, a bare string is what a
  // model sends when it ignores the schema. Neither is worth failing over.
  const list = (questions || [])
    .map((x) => (x && typeof x === 'object'
      ? { q: String(x.q || '').trim(), url: String(x.url || '').trim() }
      : { q: String(x || '').trim(), url: '' }))
    .filter((x) => x.q).slice(0, MAX_Q);
  ...
  const answers = await Promise.all(list.map(async ({ q, url }) => {
    try {
      // A named page is read; anything else is searched. A page that will not
      // load falls back to searching rather than failing the question — the
      // traveller wanted the answer, not the URL.
      const page = url && firecrawlReady() && fetchable(url) ? await scrape(url) : '';
      return { q, url, ...(await askOne(q, budget, use, page)) };
    } catch (err) { return { q, url, text: '', error: String(err.message || err).slice(0, 160) }; }
  }));
