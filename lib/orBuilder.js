...
// Which builder runs.
//
// raffy, 2026-09-05: "the building of app part. it's taking really really really
// long now. not like before. and days wrong. 7 days can become 1. photos
// missing... what u can do now is to switch it back to managed agent first while
// testing. so it behaves well when u switch back to openrouter."
//
// He is right, and the build records say why. One build made eight model calls
// that each came back with no text and no tool call at all, and was pushed back
// eight times for an itinerary it never wrote. Another called save_itinerary
// three times — and save REPLACES the trip, so the last of the three is the one
// he got. A third spent all fourteen steps on corrections and never reached
// find_photos, which is a trip with no pictures in it.
//
// So the default is the Managed Agents builder again. Set BUILDER=openrouter to
// put GLM back on it — one environment variable, once the failures below are
// fixed and a real build has been watched.
const BUILDER = () => (process.env.BUILDER || 'managed').toLowerCase();
export const orBuilderReady = () =>
  BUILDER() === 'openrouter' && !!(KEY() && firestoreConfigured());
...
async function chat(messages) {
  ...
  // it was.
  const use = data.usage || null;
  return { message: choice.message || {}, finish: choice.finish_reason, usage: use };
}
...
export async function advanceBuild(id) {
  ...
  return { itinerary: state, building: !done, error: '', usage: out.usage || null, model: MODEL(),
