...
// Turn whatever the agent called the place into something Hotellook knows.
//
// raffy, 2026-09-06, on his Kuching trip: "i ask for flight it returns answer.
// but for Sheraton hotel it says it can't and give the link instead."
//
// Flights take IATA codes, so there is nothing to get wrong. Hotels take a
// free-text place, and `cache.json` does not search for one — it wants a
// location it already holds, and answers an empty array for anything else
// rather than an error. So "Kuching, Sarawak" comes back empty while "Kuching"
// would have worked, and the empty array is indistinguishable from "this town
// genuinely has no rates". That is why the tool looked down only for hotels.
//
// `lookup.json` is the search endpoint. Ask it first, take the id of the city
// it matched, and hand `cache.json` an id instead of a guess. The id removes
// the whole class of failure: no comma parsing, no spelling, no "Sarawak".
async function findLocation(text) {
  const q = new URLSearchParams({
    query: text, lang: 'en', lookFor: 'city', limit: '1', token: token(),
  });
  const r = await fetchWith(HOTELS + '/lookup.json?' + q, T_API);
  if (!r.ok) return { bad: 'HTTP ' + r.status };
  const j = await r.json();
  const hit = (j && j.results && j.results.locations && j.results.locations[0]) || null;
  if (!hit) return { bad: 'no such place' };
  return { id: String(hit.id), name: hit.fullName || hit.cityName || text };
}
...
async function hotelPrices(q) {
  ...
  const rates = async (params) => {
    const r = await fetchWith(HOTELS + '/cache.json?' + params, T_API);
    if (!r.ok) return { bad: 'hotel rates unavailable (HTTP ' + r.status + ')' };
    const j = await r.json();
    return { list: Array.isArray(j) ? j : Object.values((j && (j.results || j.data)) || {}) };
  };
  const base = {
  ...
  const place = await findLocation(where);
  let got = place.id
    ? await rates(new URLSearchParams({ ...base, locationId: place.id }))
    : { list: [] };
  // Belt and braces: if the id route comes back empty, try the plain name and
  // then the part before the first comma. Costs one extra request on a path
  // that was returning nothing anyway.
  if (!got.bad && !got.list.length) {
    for (const name of [where, where.split(',')[0].trim()]) {
      const alt = await rates(new URLSearchParams({ ...base, location: name }));
      if (!alt.bad && alt.list.length) { got = alt; break; }
    }
  }
  if (got.bad) return got.bad;
  ...
  if (!got.list.length) {
    return [where + ', ' + q.checkIn + ' to ' + q.checkOut,
      '  no rates came back for those dates'
        + (place.bad ? ' (the provider does not know "' + where + '" as a city)' : '')
        + ' — say so rather than estimating',
      '  book: ' + hotelSearchLink(q)].join('\n');
  }
  return hotelReport(got.list, q, hotelSearchLink(q));
}
...
/**
 * One real hotel lookup, reported step by step.
 *
 * The hotel hosts are unreachable from the sandbox this app is written in, so
 * "does it work" was only answerable by asking raffy to try it in the app and
 * describe what he saw. This asks the deployment instead, and shows each stage
 * separately — did the place resolve, did rates come back, did the parser find
 * usable prices in them — because "no rates" was three different failures
...
export async function priceProbe(city, hotel) {
  ...
  const out = { asked: q };
  try {
    const place = await findLocation(q.city);
    out.lookup = place.bad ? { failed: place.bad } : { id: place.id, matched: place.name };
    const p = new URLSearchParams({
      checkIn: q.checkIn, checkOut: q.checkOut, currency: 'myr', limit: '12', token: token(),
      ...(place.id ? { locationId: place.id } : { location: q.city }),
    });
    const r = await fetchWith(HOTELS + '/cache.json?' + p, T_API);
    out.cacheStatus = r.status;
    const j = await r.json();
    const list = Array.isArray(j) ? j : Object.values((j && (j.results || j.data)) || {});
    out.rows = list.length;
    // The row's own field names, which is how the last hotel bug was found:
    // the parser was reading Hotellook names off a flight payload.
    out.firstRowKeys = list[0] ? Object.keys(list[0]).slice(0, 14) : [];
    out.report = await hotelPrices(q);
  } catch (err) {
    out.threw = String(err && err.message || err).slice(0, 200);
  }
  return out;
}
...
/**
 * Which hotel endpoint actually answers.
 *
 * raffy, 2026-09-06: flights came back, hotels did not. The probe above said
 * why — `engine.hotellook.com` returns a 404 HTML page for both `lookup.json`
 * and `cache.json`, so the host repointed to on 2026-09-06 does not serve those
 * paths at all. Guessing the replacement from memory is what produced the last
 * two wrong hosts, so this asks instead: try every candidate from the
 * deployment (which can reach them) and report what each one said.
 *
 * Delete this once the working host is settled. It exists to end a guess.
 */
export async function hotelHostProbe() {
  await loadConfig();
  const t = token();
  const CACHE_Q = 'location=Kuching&checkIn=2026-10-14&checkOut=2026-10-17&currency=myr&limit=3&token=' + t;
  const LOOK_Q = 'query=Kuching&lang=en&lookFor=city&limit=1&token=' + t;
  // Round two. Everything in round one was https and every one of them came
  // back an nginx 404 — but Travelpayouts' own documentation writes these URLs
  // as **http**, and a host that serves a path on http and 404s it on https is
  // just a different vhost answering. So the scheme is now part of what is
  // being tested, alongside the two paths the docs actually name
  // (search/start.json, tp/public/...) rather than the ones I remembered.
  const tries = [
    ['http engine cache', 'http://engine.hotellook.com/api/v2/cache.json?' + CACHE_Q],
    ['http engine lookup', 'http://engine.hotellook.com/api/v2/lookup.json?' + LOOK_Q],
    ['http yasen cache', 'http://yasen.hotellook.com/api/v2/cache.json?' + CACHE_Q],
    // Documented, and signed — a 400 or 401 here is GOOD NEWS: it means the
    // path exists and only the signature is missing.
    ['http engine search/start', 'http://engine.hotellook.com/api/v2/search/start.json?iata=KCH&checkIn=2026-10-14&checkOut=2026-10-17&adultsCount=2&token=' + t],
    ['http yasen selections', 'http://yasen.hotellook.com/tp/public/available_selections.json?language=en&token=' + t],
    // Known to exist from the docs, so this one calibrates the gateway: if it
    // answers, the prefix pattern for everything else on this host is settled.
    ['tp hotellook_static', 'https://api.travelpayouts.com/hotellook_static/v1/hotel_types?token=' + t],
    ['tp hotellook v1 cache', 'https://api.travelpayouts.com/hotellook/v1/cache.json?' + CACHE_Q],
  ];
  const out = {};
  await Promise.all(tries.map(async ([name, url]) => {
    try {
      const r = await fetchWith(url, T_API);
      const body = (await r.text()).slice(0, 160);
      out[name] = { status: r.status, body };
    } catch (err) {
      out[name] = { threw: String(err && err.message || err).slice(0, 120) };
    }
  }));
  return out;
}
