import { unlitDeep } from './blocks.js';
...
export function applyEdit(state, name, input) {
  ...
  // The same model quirk that put "RM320\\u2014360" on a chat card puts it in
  // the itinerary, where it is baked into a file somebody keeps. Absorbed once,
  // here, on the way in.
  input = unlitDeep(input);
  ...
  if (name === 'save_itinerary') {
    ...
    if (state && (state.days || []).length > 1) {
      ...
      const now = Array.isArray(input.days) ? input.days.length : 0;
      if (now < had) return state;
    }
    return reattachPhotos({
      trip: input.trip || {},
      // raffy, 2026-09-16, the Istanbul trip: one save_itinerary call sent
      // `days` as something other than an array — `|| []` never catches that,
      // because a stray object or string is truthy. Everything downstream
      // that reads `state.days` assumes an array and the whole session was
      // wedged for good: the itinerary is replayed from this same event on
      // every single request, forever, with no way back in. Checked here,
      // once, at the only place a builder's raw tool-call input becomes the
      // stored shape.
      stays: Array.isArray(input.stays) ? input.stays : [],
