...
export const EDIT_TOOL = {
  ...
  name: 'edit_itinerary',
  description:
    'Make small changes to the existing itinerary without rebuilding it. Use this for anything you can do from what you already know: changing a time, rewording something, dropping an item, adding a stop you have already researched, confirming a stay, filing a booking they have made. Call read_itinerary first to get the ids. This is instant and nearly free, so prefer it — only fall back to build_itinerary when the shape of the trip itself changes (different dates, a different city, a hotel swap that moves everything).',
  input_schema: {
    type: 'object',
    properties: {
      ops: {
        ...
        items: {
          type: 'object',
          properties: {
            op: { type: 'string', enum: ['update', 'delete', 'add', 'confirm_stay', 'save_booking'] },
