...
export function render(T, templateSrc) {
  ...
  insertBefore('</style>', [
    ...
    '  .rmap .tile{position:absolute;display:block;border:0}',
    '  .rmap .tint{position:absolute;inset:0;pointer-events:none;',
    '    background:linear-gradient(150deg,rgba(226,235,222,.62),rgba(255,252,246,.30) 45%,rgba(226,235,222,.55));',
    ...
    '  .rmap .attr{position:absolute;right:9px;bottom:8px;font-size:8.5px;color:var(--ink-faint);',
    '    background:rgba(255,255,255,.72);padding:2px 6px;border-radius:var(--r-pill);letter-spacing:.01em}',
  ...
  insertBefore('  function reduce(){', [
    ...
    '    });',
    '',
    '    // Stitched map tiles, positioned in PERCENTAGES so the whole thing',
    '    // scales with the card. No loading="lazy": these live inside a hidden',
    '    // tab, and a lazy image in a display:none subtree is never fetched at',
    '    // all. Nine small tiles do not need it.',
    '    // scales with the card. The container is locked to 640x400, so a tile',
    '    // is 40% wide and 64% tall whatever the phone.',
    '    //',
    '    // Carto rather than Wikimedia: Wikimedia maps are for Wikimedia',
    '    // projects and third-party hotlinking is restricted, which is why the',
    '    // single static image loaded on the server and not on his phone.',
    '    var tiles="";',
    '    for(var tx=Math.floor(left/TILE); tx<=Math.floor((left+MW)/TILE); tx++){',
    '      for(var ty=Math.floor(top/TILE); ty<=Math.floor((top+MH)/TILE); ty++){',
    '        if(ty<0||ty>=n) continue;',
    '        var wx=((tx%n)+n)%n;',
    '        tiles+=\'<img class="tile" alt="" src="https://basemaps.cartocdn.com/light_all/\'+',
    '          z+"/"+wx+"/"+ty+\'@2x.png" style="left:\'+((tx*TILE-left)/MW*100).toFixed(4)+\'%;top:\'+',
    '          ((ty*TILE-top)/MH*100).toFixed(4)+\'%;width:\'+(TILE/MW*100)+\'%;height:\'+(TILE/MH*100)+\'%">\';',
    '      }',
    '    }',
    '',
    '    var line="";',
    ...
    '    }).join("");',
    '',
    '    host.innerHTML=\'<div class="rmap">\'+tiles+\'<div class="tint"></div>\'+',
    '      \'<svg viewBox="0 0 \'+MW+\' \'+MH+\'" aria-hidden="true">\'+line+pins+\'</svg>\'+',
    '      \'<span class="rcap">\'+(xy.length>1?"In order":"Where you stay")+\'</span>\'+',
    '      \'<span class="attr">\\u00a9 OpenStreetMap \\u00b7 CARTO</span></div>\'+',
    '      (xy.length>1?\'<div class="rlegend">\'+xy.map(function(q){',
    '        return \'<span class="rleg"><i>\'+q.i+\'</i>\'+esc(q.n)+\'</span>\';',
    '      }).join("")+\'</div>\':\'\');',
    '',
    '    // Tiles that will not load simply vanish; the sage ground, the route',
    '    // and the labelled pins still read as the shape of the trip.',
    '    Array.prototype.forEach.call(host.querySelectorAll("img.tile"), function(im){',
