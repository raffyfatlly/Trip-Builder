...
async function hotelPrices(q) {
  ...
  const url = bookingPageFor({ ...q, hotel, city: where });
  if (!url) return nogo('could not build a page to read for those dates.');

  const n = nights(q.checkIn, q.checkOut);
  const ask = hotel
    ? 'What does ' + hotel + ' in ' + where + ' cost for ' + n + (n === 1 ? ' night' : ' nights')
      + ', ' + q.checkIn + ' to ' + q.checkOut + '?'
    : 'What do hotels in ' + where + ' cost for ' + n + (n === 1 ? ' night' : ' nights')
      + ', ' + q.checkIn + ' to ' + q.checkOut + '?';

  const out = await research([{
    url,
    about: 'room rates',
    q: ask + ' Read ONLY this page. Report a price only if the page actually shows one'
      + ' for these exact dates; if it shows no price, or only a price for other dates,'
      + ' say so plainly and give no number. Say for each property whether the figure is'
      + ' per night or for the whole stay — the page is not consistent about this and'
      + ' quoting a whole-stay total as a nightly rate makes a trip look several times'
      + ' more expensive than it is. Give the currency exactly as shown. Name at most'
      + ' four properties' + (hotel ? ', and put ' + hotel + ' first if it is there' : '')
      + '. Do not estimate, average or infer anything the page does not state.',
  }]).catch(() => null);

  // research() returns one blob of text with a "### <the question>" heading per
  // question — not a per-answer array. Reading it as an array is a silent
  // failure: every lookup would fall through to "could not read a rate" while
  // the page was fetched and paid for. Drop the heading and keep the answer.
  const body = clean((out && out.text) || '')
    .split('\n').filter((l) => !l.startsWith('### ')).join('\n').trim();
  const found = body && !/^(Nothing usable found|Could not research this)/i.test(body);
  if (!found) return nogo('could not read a rate off the booking page just now.');
  const text = body;

  return [head + '  (read off the booking page just now, not a live quote)',
    ...text.split('\n').map((l) => '  ' + l),
    '  These are published rates from one page, read at ' + new Date().toISOString().slice(0, 16).replace('T', ' ')
      + ' UTC. Rooms sell and prices move — quote them with that caveat and',
    '  send them to the link to see today\'s number.',
    '  book: ' + (link || url)].join('\n');
}
  ...
  input_schema: {
    ...
    properties: {
      ...
      hotels: {
        type: 'array',
        maxItems: 3,
        description: 'Hotel rates are READ OFF THE BOOKING PAGE, not from a price feed —'
          + ' the provider closed its hotel API. So this takes ten to twenty seconds and'
          + ' costs about a tenth of a turn: ask for it when you want a number, not'
          + ' speculatively. What comes back is what the page published at that moment,'
          + ' not a live quote, and sometimes the page shows nothing — that is a real'
          + ' answer, not a failure. Whatever it says, never estimate a rate yourself and'
          + ' never go and find one by web search: a number off a blog or an aggregator is'
          + ' not what it will cost them, and putting it on a card is worse than saying you'
          + ' do not know. Read whether each figure is PER NIGHT or for the WHOLE STAY and'
          + ' repeat it that way — quoting a stay total as a nightly rate makes a trip look'
          + ' several times more expensive than it is.',
        items: {
          type: 'object',
          properties: {
            city: {
