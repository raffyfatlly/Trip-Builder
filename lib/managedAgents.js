  ...
  for (const call of pending) {
   ...
   try {
    ...
    try {
      ...
      // TWO REASONS TO REFUSE, and the first is absolute.
      //
      // raffy, 2026-09-07: "new account with no paid credit cannot build at all.
      // it will trigger paywall." So building is what you buy — the free tier is
      // the conversation, and no amount of free credit reaches past it. Keyed on
      // whether they have ever paid rather than on the arithmetic of grant vs
      // build cost, which a change to either number could quietly reverse.
      const neverPaid = purse && !purse.unmetered && !purse.paid;
      const tooLittle = purse && !purse.unmetered && (purse.left || 0) < need;
      if (neverPaid || tooLittle) {
        results.push({
          type: 'user.custom_tool_result', custom_tool_use_id: call.id,
          content: [{ type: 'text', text: [
            neverPaid
              ? 'NOT STARTED — building the app is the paid part, and they have not bought it yet.'
