...
async function hotelPrices(q) {
  ...
  // GOOGLE HOTELS, when the free inventory has nothing. Same rule as flights:
  // last, not first, because it is a rented browser and it is billed per run.
  //
  // What it settles, which nothing else here could: the nightly rate AND the
  // whole-stay total, each labelled, plus WHO is selling at that price. Every
  // hotel rate this app has ever quoted has been one of those three short.
  ...
  if (rooms && rooms.length) {
    ...
    for (const r of rooms) {
      ...
      if (r.sources.length) {
        out.push('    on ' + r.sources.map((x) => x.site).join(', ')
          + ' — book: ' + r.sources[0].link);
      } else if (r.link) {
        out.push('    ' + r.link);
      }
    }
    ...
    out.push('  book: ' + (link || (rooms[0] && rooms[0].sources[0] && rooms[0].sources[0].link) || ''));
    return out.join('\n');
  }
