...
// the cost of serving it goes to the house ledger in lib/meter.js. The per-trip
// photo cost is attributed where it is actually incurred — the lookup that
// found the picture, in the session that asked for it — not at serve time.
// (This used to say "because /api/bake fetches them inside the session"; the
// download and its baker were removed on 2026-09-07.)
...
async function candidates(search) {
  ...
  // CHECK UNTIL THERE ARE THREE, NOT ALL ELEVEN.
  //
  // Every candidate was fetched to prove it loads, and then all but the first
  // three were thrown away. Eleven fetches to keep three, per place, eight
  // places a call — which is how one stuck Dolomites build put 1,776 requests
  // to staticflickr.com through the session before anybody noticed.
  //
  // In threes, stopping as soon as three are good: the usual case is the first
  // batch, and a place whose pictures are all dead still gets the full sweep.
  const live = [];
  for (let i = 0; i < all.length && live.length < 3; i += 3) {
    const batch = all.slice(i, i + 3);
    const okBatch = await Promise.all(
      batch.map(async (h) => (h.checked || (await loads(h.url)) ? h : null)));
    for (const h of okBatch) if (h && live.length < 3) live.push(h);
  }
...
// WHAT THE MODEL ACTUALLY SENDS, NOT WHAT THE SCHEMA ASKED FOR.
//
// The schema says `queries` is an array of objects. raffy's Penang build sent
// this instead, twice:
//
//   {"queries": "[\"Shangri-La Golden Sands Penang\", \"Clan Jetties Penang\", ...]"}
//
// A STRING holding a JSON array of BARE STRINGS — stringified once too often,
// and the objects flattened to their search terms. Both are ordinary model
// slips and neither is recoverable downstream: `.slice(0, 8)` on a string
// returns its first EIGHT CHARACTERS, so the call went off and looked up
// `[`, `"`, `S`, `h`… as if they were places. Those junk lookups were what
// hung the request, and a request that dies before it answers leaves the call
// pending forever — which is why his build ticked every phase, kept the
// progress bar spinning, and never showed the Done button. Two calls held it
// hostage, and the twelve places inside them never got a photograph either.
//
// So the shape is repaired here rather than insisted upon. A tool that only
// works when the model formats perfectly is a tool that fails on the trips
// that matter.
function asQueries(queries) {
  let q = queries;
  if (typeof q === 'string') {
    const t = q.trim();
    try {
      const parsed = JSON.parse(t);
      q = Array.isArray(parsed) ? parsed : [parsed];
    } catch (e) {
      // Not JSON. One search, said plainly, beats eight characters.
      q = t ? [t] : [];
    }
  }
  if (!Array.isArray(q)) q = q ? [q] : [];
  return q.map((x) => {
    if (typeof x === 'string') return { key: fillKey(x) || x.slice(0, 40), search: x };
    if (x && typeof x === 'object') {
      // A key is what the itinerary will refer to. Derive one when it is
      // missing rather than answering under `undefined`.
      const key = x.key || fillKey(x.search || x.page || x.url || '') || '';
      return { ...x, key };
    }
    return null;
  }).filter((x) => x && (x.search || x.url || x.page));
}
...
export async function findPhotos(queries) {
  const list = asQueries(queries).slice(0, 8);
  ...
  // THE BUDGET HAS TO BE ABLE TO END THE CALL, not merely be noticed once it
  // is over. It was checked AFTER the await, so a lookup that never came back
  // took the whole request with it — past the platform's ceiling, killed
  // mid-flight, the tool result never sent, the builder left holding the call
  // for good. A photo search must never be able to do that.
  ...
  const results = await Promise.all(list.map(async (q) => {
    try {
      return {
        ...
        ...(await Promise.race([
          resolveOne(q),
          new Promise((r) => setTimeout(() => r({ hits: [], note: 'this one took too long — skipped' }), T_ALL)),
        ])),
      };
    } catch (err) {
...
export function photoGaps(it) {
  const out = [];
  const seen = new Set();
  // `sure` means "this is a place, by definition" — a STAY. The two filters
  // below exist to stop us paying Google to look up a description of an
  // afternoon ("Slow start", "Head back to the hotel"), and they are the right
  // filters for a day item. On a stay they are wrong, and expensively so:
  // looksLikeAPlace demands a capital letter after a space, which a one-word
  // hotel can never have. "Sofitel", "Yotel", "Ibis", "Oasia" were all skipped
  // — no photo, and no coordinates either, so no pin on the map.
  //
  // raffy, 2026-09-07, on Syahirah's Singapore trip: "image not reflected in
  // app (hotel image)". A hotel is a place because it is a hotel. Nothing about
  // its name has to prove it.
  const want = (name, o, sure) => {
    ...
    if (!sure && LOGISTICS.test(n)) return;
    if (!sure && !looksLikeAPlace(n)) return;
    const k = fillKey(n);
  ...
  for (const st of (it && it.stays) || []) want(st.n || st.short, st, true);
...
// Roughly how far apart two points are, in km. Good enough to tell Tokyo from
// Montreal, which is the entire job.
const kmApart = (a, b) => {
...
// Anything this far from every stay is not on this trip.
//
// raffy's Tokyo trip, 2026-09-05: "the map too huge. they take the world map."
// One item — "Grand Sumo Tournament" — was geocoded to 45.47N, -73.44W, which
// is Montreal. The map then had to span Tokyo to Canada, so it drew the world
// and every real pin was a dot.
//
// A place name without a city in it can match anywhere, and a search that comes
// back confident and wrong is worse than one that finds nothing. So a filled
// coordinate has to be plausibly on the trip: 400km covers a day trip out of
// any base, several bases in one country, and nothing on another continent.
// The photo is kept either way — a picture of the wrong sumo hall is a bad
// picture; a pin in Canada breaks the map for everything else on it.
const FAR_KM = 400;

const anchors = (it) => ((it && it.stays) || [])
...
export function applyFill(it, fill) {
  ...
  const home = anchors(it);
  const onThisTrip = (lat, lon) =>
    !home.length || home.some((h) => kmApart(h, { lat, lon }) <= FAR_KM);
  let used = false;
  const take = (name, o) => {
    if (!o) return o;
    const r = fillOf(fill[fillKey(name)]);
