...
export const marker = () =>
  ...
  || (CONFIG && CONFIG.travelpayoutsMarker) || '';
...
// start pays one Firestore read; every call after that pays nothing.
let CONFIG = null;
let CONFIG_AT = 0;
const CONFIG_TTL = 300000;

export async function loadConfig() {
  if (CONFIG && Date.now() - CONFIG_AT < CONFIG_TTL) return CONFIG;
  try {
    const { readConfig, firestoreConfigured } = await import('./firestore.js');
    CONFIG = firestoreConfigured() ? await readConfig() : {};
  } catch (err) {
    // No store, or it is down. Prices go quiet; nothing else is affected.
    CONFIG = CONFIG || {};
  }
  CONFIG_AT = Date.now();
  return CONFIG;
...
export const token = () =>
  process.env.TRAVELPAYOUTS_TOKEN || (CONFIG && CONFIG.travelpayoutsToken) || '';
