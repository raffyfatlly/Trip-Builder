 ...
 *
 * `alsoTry` is an extra key checked before spending money, read from the
 * namespace given as `alsoTryNs` (the default, unnamespaced one — lookupPlace's).
 * That is how a photo search reuses a lookup the chat already paid for.
 *
...
export function cached(query, fetch, opts = {}) {
  ...
  const spare = alsoTry ? tag(alsoTryNs, placeKey(alsoTry)) : '';
  const keys = spare && spare !== mine ? [mine, spare] : [mine];
