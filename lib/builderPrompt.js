...
- **Give every idea a \`photo\`, a \`rating\` and a \`price\`** wherever you have them. Attach the picture with \`add_photos\` using \`target: "idea"\` and the idea's index, the same way you attach one to a stay. Explore is browsed by picture — an idea with none of the three is a line of text in a list nobody scrolls. Look the photos up with find_photos in the same pass as the stays.
...
- **trip.notes** is for what they should check before locking the trip in: anything unverified, anything unbooked, anything time-sensitive. **A note about one specific stay not being booked yet must carry that stay's \`stay\` index.** It then clears itself the moment the traveller confirms that stay — untagged, it would sit there claiming "not booked" forever, even after they book it. General caveats (a price you could not verify, a season note) have no stay and stay as they are.
