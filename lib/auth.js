...
// A missing secret must not silently produce forgeable tokens, so it falls
// back to a per-deployment value rather than to a constant. Sessions then end
// on redeploy, which is the right failure: annoying, not insecure.
const FALLBACK = 'trip-builder/' + (process.env.VERCEL_DEPLOYMENT_ID || process.env.VERCEL_URL || 'local');
