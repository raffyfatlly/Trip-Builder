...
export async function pushPrompt() {
  ...
  try {
    ...
    const res = await fetchWith(API + '/v1/agents/' + CHAT_AGENT_ID, T, {
      method: 'POST',
      headers: headers(),
      // live.tools and live.model, verbatim. Whatever he set stays set.
      body: JSON.stringify({ system: SYSTEM, tools: live.tools, model: live.model }),
    });
    if (!res.ok) return 'push failed (' + res.status + ' ' + (await res.text()).slice(0, 200) + ')';
    const after = await res.json();
    const same = JSON.stringify(after.tools) === JSON.stringify(live.tools)
      && JSON.stringify(after.model) === JSON.stringify(live.model);
    return 'prompt pushed as v' + after.version
      + (same ? ', tools and model untouched' : ' — WARNING: tools or model changed');
  } catch (err) {
