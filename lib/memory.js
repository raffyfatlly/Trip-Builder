...
export const REMEMBER_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      name: { type: 'string', description: 'What to call them — their own first name, as they gave it.' },
...
// Not a slot yet, deliberately. raffy, 2026-09-01: "make sure all in RM. maybe
// in future we can make it as currency preference in profile, but now agent
// just give any price in RM." So RM is written into the prompts rather than
// stored per person; when it becomes a preference it belongs here, as a
// `currency` slot, and the prompts read it from memoryBlock like everything
// else.
...
export function applyMemory(memory, name, input, now = Date.now()) {
  ...
  for (const f of ['name', 'home', 'dietary', 'pace', 'interests', 'budget']) {
...
// How many things we actually know. Drives the nudge to save a profile that
// has grown enough to be worth keeping.
export const filledCount = (m) =>
  !m ? 0 : FIELDS.filter((f) => m[f] && (!Array.isArray(m[f]) || m[f].length)).length;

// --- editing it by hand ---------------------------------------------------
//
// The agent fills most of this in, but it is the traveller's own profile and
// they have to be able to correct it. Everything is a line of text in the UI,
// including the people — "Aisyah, Adam (6), Nur (3)" reads better than three
// rows of form fields, and it round-trips.

export const peopleText = (people, now = Date.now()) =>
  (people || []).map((p) => {
    const a = ageNow(p, now);
    return p.name + (a != null ? ` (${a})` : '') + (p.note ? ` — ${p.note}` : '');
  }).join(', ');

export function parsePeople(text, now = Date.now()) {
  return String(text || '')
    .split(',')
    .map((chunk) => {
      const raw = chunk.trim();
      if (!raw) return null;
      const [, before, note] = raw.match(/^(.*?)(?:\s+[—-]\s+(.*))?$/) || [];
      const m = (before || '').match(/^(.*?)\s*\((\d{1,2})\)\s*$/);
      const p = { name: (m ? m[1] : before || '').trim() };
      if (!p.name) return null;
      if (m) p.age = parseInt(m[2], 10);
      if (note) p.note = note.trim();
      return cleanPerson(p, now);
    })
    .filter(Boolean)
    .slice(0, 12);
}

// Apply one hand edit. Returns the new profile, or the old one unchanged when
// the text was empty — clearing a field is what the remove button is for, so
// an accidental blur cannot wipe a line.
export function editSlot(memory, key, text, now = Date.now()) {
  if (!FIELDS.includes(key)) return memory;
  const t = String(text || '').trim();
  if (!t) return memory;
  const m = { ...(memory || {}), v: MEMORY_VERSION, at: now };
  if (key === 'people') {
    const people = parsePeople(t, now);
    if (!people.length) return memory;
    m.people = people;
  } else if (key === 'notes') {
...
export function memoryBlock(memory, now = Date.now()) {
  ...
  if (memory.name) lines.push('- They are called ' + memory.name);
