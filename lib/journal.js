...
async function edit(session, fn) {
  if (!session || !firestoreConfigured()) return null;
...
export function note(session, ev, data) {
 ...
 */
export function spendTotal(session, provider, model, u) {
  return edit(session, (j) => {
    j.spend[provider] = {
      model: model || '',
      in: u.in || 0,
