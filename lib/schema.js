...
const stay = {
  ...
  properties: {
    ...
    draft: { type: 'boolean', description: 'true if this booking is not confirmed. This is the ONLY place to say so — the app shows it and clears it when the traveller confirms. Never also put "not booked" in a tag or in prose.' },
    photo: { type: 'string' },
    credit: { type: 'string' },
    licence: { type: 'string' },
    ...
    notes: {
      ...
      items: {
        ...
        properties: {
          ...
          stay: { type: 'integer', description: 'If this note is ABOUT one specific stay not being booked yet, its index. The note then disappears on its own once the traveller confirms that stay — leave it out for anything else.' },
        },
        required: ['kind', 'h', 'p'],
      },
    },
