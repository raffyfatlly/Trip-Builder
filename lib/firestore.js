...
//
// Capped and short-lived on purpose. This is a holding pen between two agent
// turns, not a chat archive — the conversation's real record is the session's
// own event log, which these join the moment the agent next speaks.
const HELD_MAX = 40;
...
export async function appendHeld(session, message) {
  ...
  const messages = [...now, message].slice(-HELD_MAX);
...
export async function writeLedger(l) {
  const mask = ['granted', 'used', 'plan', 'build', 'since'].map((f) => 'updateMask.fieldPaths=' + f).join('&');
