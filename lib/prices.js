...
export function hotelSearchLink({ where, checkIn, checkOut, adults = 2 }) {
  const w = clean(where);
  if (!w) return '';
    ...
    properties: {
      ...
      hotels: {
        ...
        items: {
          type: 'object',
          properties: {
            where: { type: 'string', description: 'City, area or hotel name.' },
...
export async function checkPrices(input) {
  ...
  if (!pricesReady()) {
    ...
    const links = [
      ...
      ...hotels.map((h) => 'hotels in ' + h.where + ': ' + (hotelSearchLink(h) || 'could not build a link')),
    ];
    return ['Live prices are not configured, so I cannot quote a fare. Do NOT estimate one.',
