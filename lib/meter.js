...
const BOOK = [
  ...
  // Firecrawl's published pricing is per credit rather than per request and is
  // not stated in their onboarding doc, so this counts the calls at zero rather
  // than inventing a rate. Volume is the thing worth seeing first; put a real
  // number here once a bill has been seen.
  { host: 'api.firecrawl.dev', service: 'firecrawl', usd: 0 },

  // A rented browser, billed per run and by far the most expensive per-call
  // thing here — which is exactly why it is the last resort rather than the
  // first. The figure is a placeholder until a real Apify bill has been seen;
  // the actor's own pricing (?apifystore= on the health endpoint reports it)
  // is what should replace it. Counting it high is the safe direction: it
  // makes a run show up in the ledger rather than hide in it.
  { host: 'api.apify.com', service: 'apify', usd: 0.02 },

  { host: 'routes.googleapis.com', service: 'routes', usd: 0.005 },
  { host: 'maps.googleapis.com', service: 'maps', usd: 0.005 },

  // NOT TRACKED. raffy, 2026-09-07: "just removed if it's not helping and always
  // showing error (journal). just track the most solid one."
  //
  // These are free, and counting them was not merely noise — metering Firestore
  // into a journal that IS a Firestore document is a feedback loop. Every read
  // metered a call, which queued a journal write, which was another call, which
  // metered again. One KL session logged 949 firestore calls and 230 failed
  // ones, and the 15-second timeouts flooding the error log were that loop
  // failing to drain. Cutting it removes the loop and the log noise together.
  //
  // They stay in the book so `look()` still recognises the host and does not
  // file it under "other:", which is the column that has to mean something.
  { host: 'firestore.googleapis.com', service: 'firestore', usd: 0, track: false },
  { host: 'storage.googleapis.com', service: 'storage', usd: 0, track: false },
  { host: 'oauth2.googleapis.com', service: 'auth', usd: 0, track: false },
  { host: 'maps.wikimedia.org', service: 'maptile', usd: 0, track: false },
  { host: 'commons.wikimedia.org', service: 'commons', usd: 0, track: false },
  { host: 'api.openverse.org', service: 'openverse', usd: 0, track: false },
  { host: 'api.open-meteo.com', service: 'weather', usd: 0, track: false },
  { host: 'archive-api.open-meteo.com', service: 'weather', usd: 0, track: false },
  { host: 'date.nager.at', service: 'holidays', usd: 0, track: false },
  { host: 'open.er-api.com', service: 'fx', usd: 0, track: false },
  { host: 'api.travelpayouts.com', service: 'affiliate', usd: 0, track: false },
  { host: 'locktrip.com', service: 'locktrip', usd: 0, track: false },
];
...
function look(url) {
  ...
  if (loose) return loose.track === false ? null : loose;
  ...
  //
  // Except the free asset hosts the map and the photos pull from. Those filled
  // the journal with other:thumb.wikimedia.org rows that will never cost a
  // cent, and a report where every session has four junk rows is a report
  // nobody reads.
  if (/(^|\.)(wikimedia\.org|wikipedia\.org|gstatic\.com|googleusercontent\.com|imagecontent\.net)$/i
