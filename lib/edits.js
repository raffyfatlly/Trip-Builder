...
export const OPS = [
  ...
  // The trip header — start, end, title. It exists so a trip can MOVE to
  // different dates without being rebuilt. See lib/dates.js: a trip that
  // shifts by a week is the same trip, and until this op there was no way to
  // say so, which is why the agent rebuilt from scratch and drained the first
  // real user's balance.
  'trip.update',
  // Whole days, added and removed. Until these existed, "make it a day longer"
  // and "drop the Tuesday" were rebuilds — not because the day is hard, but
  // because every date after it moves. That is arithmetic; see redate() in
  // lib/dates.js, which runs automatically whenever one of these lands.
  'day.add', 'day.delete',
...
// `item.delete` nulls a slot in place rather than splicing it out — the
// filter that removes nulls runs once, after every op in the batch, so a
// day that is touched twice (delete one item, then edit another) still has
// null sitting in the middle while later ops in the SAME batch are applied.
// `it && ` is the whole fix: without it, `it._id` on that null slot is a
// TypeError that takes down both the render (a blank page — see
// pages/index.js's `applyEdits` call in its own `useMemo`) and /api/advance,
// wherever this ran server-side too. raffy, 2026-09-14, on Melbourne after
// changing a hotel and a location in one go: "the app got stuck giving the
// blank page" — that is two ops touching one day, one of them a delete,
// landing in the one batch this line could not survive.
const findItem = (day, id) => (day.items || []).findIndex((it) => it && it._id === id);
...
export function applyEdits(base, edits) {
  ...
  let added = 0;
  // Whether the number of days changed, and therefore whether every date
  // downstream has to be worked out again.
  ...
  for (const op of edits) {
    ...
    if (op.type === 'day.add') {
      const days = out.days || (out.days = []);
      const at = Math.max(0, Math.min(days.length, op.at));
      // Items arriving with a new day need ids of their own: withIds() ran on
      // the base, before this day existed, so nothing here has been through it.
      const items = ((op.day && op.day.items) || []).map((x, i) => ({
        ...clone(x), _id: (op.id || 'nd' + op.ts) + '-' + i,
      }));
      days.splice(at, 0, { stay: 0, title: '', sub: '', ...clone(op.day || {}), items });
      restructured = true;
      continue;
    }

    if (op.type === 'day.delete') {
      const days = out.days || [];
      if (op.at >= 0 && op.at < days.length && days.length > 1) {
        days.splice(op.at, 1);
        restructured = true;
      }
      continue;
    ...
    if (op.type === 'trip.update') {
      out.trip = { ...(out.trip || {}), ...op.patch };
      continue;
    }
  ...
  // A day added or dropped moves every date after it. Derived here rather than
  // asked of whoever made the edit, so it cannot be got wrong or forgotten.
  if (restructured) redate(out);
...
export function countStale(base, edits) {
  ...
  for (const op of edits) {
    ...
    // A task or a booking is never orphaned: neither is addressed by a day id,
    // which is the only thing a rebuild invalidates. That is most of the reason
    // they are the right home for anything that has to survive one.
    //
    // This branch used to hold a copy of applyEdits' task handler, pasted into
    // a function that has no `out` — so the first task edit to reach it threw
    // ReferenceError and took the whole page down with it. It sat unreachable
    // until to-dos started being routed as tasks, then broke the app on load.
    if (op.type === 'task.set' || op.type === 'task.clear'
      || op.type === 'booking.set' || op.type === 'booking.clear') continue;
