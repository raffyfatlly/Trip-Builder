 ...
 */
/**
 * Has this line already been written?
 *
 * DEDUPE NAMES A FIELD IN `data`, and a name that is not there used to mean "no
 * dedupe" — silently. 2026-09-09: a diagnostic passed 'pump' as the key while
 * its data carried no `pump` field, so every single call appended. Four hundred
 * lines went into one session's journal before anybody looked at it.
 *
 * Passing a key always means "only one of these". An absent key therefore falls
 * back to one line per EVENT rather than none — the safe direction, since the
 * caller has already said it wants collapsing. No key at all still means keep
 * everything: a conversation's timeline is not a set.
 *
 * Exported so the rule can be tested without a database, which is the only
 * reason this is a named function rather than four lines inline.
 */
export function alreadyThere(lines, ev, dedupe, data) {
  if (!dedupe) return false;
  const rows = lines || [];
  return (data && data[dedupe] != null)
    ? rows.some((l) => l.ev === ev && l[dedupe] === data[dedupe])
    : rows.some((l) => l.ev === ev);
}

export function note(session, ev, data, dedupe) {
  return edit(session, (j) => {
    if (alreadyThere(j.lines, ev, dedupe, data)) return;
    const line = { t: now(), ev };
...
export function addMetered(session, tally, who) {
  ...
  return edit(session, (j) => {
    // FIRST signer wins, for the life of the session.
    //
    // raffy, 2026-09-07: "I use her phone, log out, then sign in again suddenly
    // her session is saved on my account." This line used to be an unconditional
    // assignment, so the session's recorded owner became whoever was signed in
    // on the most recent request — and settle() bills `j.who`, so the money
    // followed too. His wife's Singapore session ended up recorded against his
    // email while the trip sat on hers.
    //
    // An anonymous session that later signs in still gets its owner set, which
    // is the one transition that SHOULD happen: '' -> somebody. After that it
    // is fixed. lib/firestore.js owners/<session> is the authority; this is the
    // same fact cached on the journal so billing needs no extra read.
    if (who && !j.who) j.who = short(who);
    for (const k of rows) {
      const t = tally[k];
 ...
 */
// WHICH ROWS ARE THE BUILD.
//
// 2026-09-08, reconciling the Penang trip: the ledger read `plan 99, build 0`
// even though the builder agent had cost $0.33 on that trip. The meter writes
// the managed builder agent under `builder.agent` (managedAgents.js), and this
// split matched the bare key `builder` — so every dollar the builder spent was
// filed as "other", and "other" is then apportioned by the model spend either
// side, which was itself zero for the build. Both halves of the mistake pointed
// the same way, so the ring showed a build that never happened.
//
// Prefix match, so a second builder row (a different model, a retry) lands on
// the right side of the split without this having to be edited again. Nothing
// about what is CHARGED depends on it — only the ring the traveller reads.
const isBuild = (k) => k === 'builder' || k.startsWith('builder.');

export async function journalSettle(session, convert) {
  let out = null;
  await edit(session, (j) => {
    const rows = j.spend || {};
    const was = j.chargedRows || {};
    const now = {};
    let delta = 0, plan = 0, build = 0, other = 0;

    // Per row rather than one total, so the charge can be split into something
    // a traveller understands without a second set of books. The TOTAL is still
    // the authority; the split is derived from it and always sums back to it.
    for (const k of Object.keys(rows)) {
      const usd = Number(rows[k].usd) || 0;
      now[k] = usd;
      const d = usd - (Number(was[k]) || 0);
      if (d <= 0) continue;          // a row can only ever go up
      delta += d;
      // Research is planning: it is what the conversation does to answer a
      // question, and a traveller reading the ring thinks of it that way.
      if (k === 'chat' || k === 'research') plan += d;
      else if (isBuild(k)) build += d;
      else other += d;               // Places, Routes, maps
    }

    // Rounded before converting. Summing floats and then multiplying by a rate
    // put the charge one credit under what the same dollars converted directly
    // would give — small, and the kind of drift that makes two reports of the
    // same money disagree.
    delta = +delta.toFixed(6);
    // A tenth of a cent. Below this the Firestore write costs more attention
    // than the money is worth, and it is picked up on the next request.
    if (delta < 0.001) return;

    // CHARGE ON THE RUNNING TOTAL, NOT ON EACH INCREMENT.
    //
    // raffy, 2026-09-07, on his wife's Singapore trip: her session cost $2.838
    // — 41 credits — and she was charged 58 against a grant of 70. She ran out,
    // and it was not because the trip was expensive.
    //
    // convert() rounds UP, deliberately: a fractional credit rounded down is a
    // rounding error in our favour on every request, and there are a lot of
    // requests. That reasoning is right and the implementation inverted it.
    // settle() runs after every metered request, so rounding up was applied to
    // each increment SEPARATELY — about twenty times in her session, each one
    // adding up to a whole credit that was not spent. 41 became 58: a 41%
    // surcharge on every user, invisible in any single charge.
    //
    // Rounding up once, on the cumulative total, keeps the intent (nobody is
    // ever charged less than they cost) and costs a person at most ONE extra
    // credit for a whole session rather than one per request.
    const totalSoFar = +Object.values(now).reduce((a, b) => a + b, 0).toFixed(6);
    const owed = convert(totalSoFar) - (Number(j.chargedCredits) || 0);
    // Nothing has crossed a whole credit yet. Deliberately does NOT advance
    // chargedRows, so this spend is counted again next time and accumulates
    // until it does — the alternative is losing it.
    if (owed <= 0) return;
    const credits = owed;

    // Where the services go.
    //
    // The meter counts them per session, not per phase, so there is no exact
    // answer to "was this photo lookup the conversation or the build" — the
    // chat looks places up for its cards and the builder looks them up for its
    // pictures, and one Firestore document holds both. They are apportioned by
    // the model spend either side, which is a good approximation of who was
    // working, and matters only to the ring the traveller sees. Nothing about
    // what is CHARGED depends on it.
    const chat = (Number((rows.chat || {}).usd) || 0) + (Number((rows.research || {}).usd) || 0);
    const bld = Object.keys(rows).reduce((a, k) => a + (isBuild(k) ? (Number(rows[k].usd) || 0) : 0), 0);
    const share = chat + bld > 0 ? bld / (chat + bld) : 0;
    build += other * share;
    plan += other * (1 - share);

    // Split the credits rather than converting each part, so rounding cannot
    // make the two halves disagree with the total that was actually taken.
    const buildCredits = Math.min(credits, Math.round(credits * (build / delta)));
    const planCredits = credits - buildCredits;

    j.chargedRows = now;
    j.chargedUsd = +Object.values(now).reduce((a, b) => a + b, 0).toFixed(6);
    j.chargedCredits = (Number(j.chargedCredits) || 0) + credits;
    out = { who: j.who || '', usd: delta, credits, plan: planCredits, build: buildCredits };
  });
  return out;
}
