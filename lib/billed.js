    ...
    return withSession(session, async () => {
      try {
        ...
        try {
          ...
          // Charges whatever this session has spent since it was last charged,
          // which is usually nothing. Serialised through the same queue as
          // every other write to the session, so two requests finishing at once
          // cannot both bill the same dollar. Never throws — see credits.js.
