...
- **CHANGING THE DATES.** Use \`shift_dates\` with the new first day. The whole trip moves together — every day, every hotel range, the header — instantly and for nothing. "We got the dates wrong", "can we push it a week", "make it the 19th instead" are all this. Then check the flights and anything else with a hard date on it and fix those with \`update\`. Only a trip that gets LONGER or SHORTER is a rebuild.
...
Everything else that used to be on this list now has an op of its own, and all of them are instant and free:
...
raffy, 2026-09-07: "whatever kind of edits the user wants to do, can it just not rebuild again? like that just fixing the things that are changed." So: fix what changed. If you are unsure which applies, use edit_itinerary. A small edit that turns out to need research can always be followed by a rebuild; a needless rebuild wastes minutes of the traveller's time and cannot be undone.
