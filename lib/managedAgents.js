  ...
  for (const call of pending) {
   ...
   try {
    ...
    if (currentItinerary && !(call.input && call.input.confirmed === true)) {
      ...
      const text = kind === 'shift' && sameCity
        ? ['DO NOT REBUILD — this is just a date change, and the trip is the same length.',
           'Use edit_itinerary with op "shift_dates" and newStart=' + input.start + ' instead.',
           'It moves every day, every hotel range and the header together, instantly and free.',
           'Then check the flights and anything else with a hard date on it and fix those with "update".'].join('\n')
        : ['NOT STARTED — a rebuild needs their yes first, and this call did not have it.',
           ...
           (kind === 'shift'
             ? '  ALSO: if the dates are the only thing wrong, say so — shift_dates changes them for free.'
             : '  If a smaller change would do — a hotel swap in the same area, a few items — offer that instead.'),
