...
export async function findOrCreate({ email, phone }) {
  if (backend() === 'firestore') {
    ...
    return writeAccount({ email, phone: phone || '', trips: [] });
  }
...
export async function saveTrips(email, trips) {
  ...
  if (backend() === 'firestore') {
    const acct = (await readAccount(email)) || { email, phone: '', trips: [] };
...
async function supaGet(email) {
  const rows = await rest('accounts?email=eq.' + encodeURIComponent(email) + '&select=email,phone,trips');
  const r = (rows || [])[0];
  return r ? { email: r.email, phone: r.phone || '', trips: r.trips || [] } : null;
}
