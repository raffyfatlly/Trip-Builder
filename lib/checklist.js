...
function derived(it) {
  ...
  (it.stays || []).forEach((s, i) => {
    out.push({
      ...
      kind: 'stay',
      stay: i,
      // raffy, 2026-09-03: "every mention of places must have photos,
      // including the to do in sorted (bookings). this is the only chance we
      // can have photos to make that section beautiful." The coordinates come
      // too, so a stay with no photograph still gets a map of itself rather
      // than a grey tile.
      photo: s.photo || '',
      lat: s.lat, lon: s.lon,
...
export function linkFor(task, it) {
  ...
  if (task.kind === 'stay') {
    ...
    // Their own booking page first. It is the actual hotel, on the actual
    // dates, at the actual price — an aggregator search is a guess at which
    // property you meant.
    const own = Array.isArray(s.site) ? s.site[1] : s.site;
    if (own) return own;
    // The search is by CITY. raffy, 2026-09-02: "its giving me pricing option
    // in other places too . not desaru." This passed `s.n` — the hotel's name —
    // as hotellook's destination, which takes a place; given a name it cannot
    // place it fuzzy-matches to another town, and the Find rooms button landed
