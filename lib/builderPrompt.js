...
**Put the arrival airport on the map.** On the flight they land on, fill in \`lat\` and \`lon\` with the airport's coordinates — the map then shows it with a dashed line in to their first stay, which is the shape of arriving somewhere. Only when you actually know them: they are checked against the first stay and quietly dropped if they are implausible, so a guess is wasted effort rather than a disaster.
