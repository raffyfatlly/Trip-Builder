...
// A PHONE NUMBER IS A BUTTON ON A PHONE.
//
...
// On the device this app is actually used on, a number you cannot tap is a
// number you have to memorise, switch apps for, and retype.
//
// Deliberately tight, because a loose phone pattern eats dates, flight numbers
// and reference codes. It wants a Malaysian-shaped number: an optional +60,
// then a 0-led area code, a dash, and 6-8 digits in one or two groups.
const PHONE = /(?:\+?60[-\s]?|0)\d{1,2}[-\s]\d{3,4}[-\s]?\d{3,4}\b/g;

const TOKEN = new RegExp([
  // [label](url) — now tel: as well as http(s), so the agent can label a
  // number ("Call the hotel") instead of printing digits.
  /\[([^\]\n]{1,80})\]\(((?:https?:\/\/|tel:)[^\s)]+)\)/.source,
  /\*\*([^*\n]+?)\*\*/.source,                            // **bold**
...
// SITES PEOPLE KNOW BY NAME, and the hosts they actually live on.
//
// raffy, 2026-09-08, tapping a button that said "Book on Google Flights" and
// landing on Aviasales: "the link must correspond to what it says."
//
// The label is written by the agent and the URL is built by the app, so nothing
// was checking that the two agreed — and when they disagree, the label is the
// lie, because the traveller reads it and the browser obeys the URL. Naming the
// wrong site is worse than naming none: it is a promise about where a tap goes.
//
// So a label that names a site it is not going to gets corrected here, in the
// one place every link in the app is drawn. The rest of the label is left
// exactly as written — "Book the 9:45 PM flight on Google Flights" keeps its
// sentence and only the site name moves.
const SITES = [
  [/google\s*flights?/i, 'Google Flights', /(^|\.)google\.[a-z.]+$/i, /travel\/flights|flights/i],
  [/google\s*hotels?/i, 'Google Hotels', /(^|\.)google\.[a-z.]+$/i, /travel\/hotels|hotels/i],
  [/booking\.?com/i, 'Booking.com', /(^|\.)booking\.com$/i],
  [/agoda/i, 'Agoda', /(^|\.)agoda\.com$/i],
  [/expedia/i, 'Expedia', /(^|\.)expedia\.[a-z.]+$/i],
  [/aviasales/i, 'Aviasales', /(^|\.)aviasales\.[a-z.]+$/i],
  [/skyscanner/i, 'Skyscanner', /(^|\.)skyscanner\.[a-z.]+$/i],
  [/kayak/i, 'KAYAK', /(^|\.)kayak\.[a-z.]+$/i],
  [/trip\.com/i, 'Trip.com', /(^|\.)trip\.com$/i],
  [/airbnb/i, 'Airbnb', /(^|\.)airbnb\.[a-z.]+$/i],
  [/priceline/i, 'Priceline', /(^|\.)priceline\.com$/i],
];

/** What site this URL really goes to, named the way a person would say it. */
export function siteOf(url) {
  ...
  for (const [, name, hostRe, pathRe] of SITES) {
    if (hostRe.test(host) && (!pathRe || pathRe.test(path))) return name;
  }
  return host.replace(/^www\./, '');
}

/** Correct a label that names a site the link does not go to. */
export function honestLabel(url, label) {
  const text = String(label || '');
  const real = siteOf(url);
  if (!text || !real) return text;
  for (const [claim, name] of SITES) {
    if (!claim.test(text)) continue;
    if (name === real) return text;            // it said where it goes. Good.
    // It named a different site. Swap the name rather than throwing the whole
    // label away — what they were being asked to DO is still useful.
    return text.replace(claim, real);
  }
  return text;
}
...
export function linkLabel(url, label) {
  const clean = honestLabel(url, String(label || '').trim());
  if (clean) return clean;
  // A tel: URL has no hostname, so the URL branch below would label it with an
  // empty string and render a chip with nothing in it. The number IS the label.
  if (/^tel:/i.test(String(url))) return String(url).slice(4);
  try {
    const u = new URL(url);
...
export function tokens(text) {
  ...
  // Prices and phone numbers, on whatever is not a link or bold.
  //
  // Prices first: a currency-prefixed figure is never a phone number, and
  // running the tighter pattern first keeps "RM2,113 1888" from being read as
  // one. Whatever the price pass leaves as text is offered to the phone pass.
  const phones = (str) => {
    let last = 0;
    let m;
    PHONE.lastIndex = 0;
    while ((m = PHONE.exec(str))) {
      if (m.index > last) out.push({ t: 'text', v: str.slice(last, m.index) });
      out.push({ t: 'link', href: 'tel:' + m[0].replace(/[^\d+]/g, ''), v: m[0] });
  ...
  const plain = (str) => {
    ...
    while ((m = PRICE.exec(str))) {
      if (m.index > last) phones(str.slice(last, m.index));
      out.push({ t: 'price', v: m[0] });
      last = m.index + m[0].length;
    }
    if (last < str.length) phones(str.slice(last));
  };
