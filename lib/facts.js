...
import { checkPriceSource } from './prices.js';
...
/**
 * One number: how many ringgit a unit of `code` is worth today.
 *
 * rateTo() above formats this for the agent to read. prices.js needs the figure
 * itself, because LockTrip quotes in dollars and a traveller in Kuala Lumpur
 * thinks in ringgit. Returns 0 when it cannot be established — and 0 must never
 * become a converted price, only an unconverted one with its unit named.
 */
export async function toMyr(code) {
  const cc = String(code || '').toUpperCase();
  if (cc === 'MYR') return 1;
  if (!/^[A-Z]{3}$/.test(cc)) return 0;
  try {
    const r = await fetchWith(FX + cc, 6000);
    if (!r.ok) return 0;
    const j = await r.json();
    return num(j && j.rates && j.rates.MYR) || 0;
  } catch (err) { return 0; }
}
...
export async function checkSources() {
  ...
  const checks = await Promise.all([
    ...
    ping('prices (travelpayouts)', async () => checkPriceSource()),
