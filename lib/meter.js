...
function look(url) {
  ...
  // Anything not in the book is still counted, under its own hostname, at zero.
  // A service nobody priced showing up in the report is the point: it is how
  // the next unbilled thing gets noticed.
  return loose || { host: u.hostname, service: 'other:' + u.hostname, usd: 0, unknown: true };
}
