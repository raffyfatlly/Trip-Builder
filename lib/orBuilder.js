...
export async function startBuild(briefText, session) {
  ...
  await writeBuild(id, {
    ...
    error: '',
    // How long a build actually takes was not recorded anywhere, so answering
    // "why is the second rebuild slower" meant reading raw event logs by hand
    // (2026-09-06). The chat session comes along so the finished build can
    // write itself into that session's journal.
    startedAt: Date.now(),
    session: session || '',
  });
  return id;
...
// Written once, when a build stops for any reason. Fire and forget: a journal
// write must never be the thing that fails a build.
function logBuild(b, extra) {
  try {
    if (!b || !b.session) return;
    const it = extra.itinerary || b.itinerary;
    note(b.session, 'build', {
      seconds: b.startedAt ? Math.round((Date.now() - b.startedAt) / 100) / 10 : 0,
      steps: extra.steps == null ? b.steps : extra.steps,
      max: MAX_STEPS,
      via: 'openrouter',
      model: MODEL(),
      capped: extra.capped ? 1 : 0,
      failed: extra.error ? 1 : 0,
      why: extra.error || '',
      days: it && it.days ? it.days.length : 0,
      items: it && it.days ? it.days.reduce((n, d) => n + ((d.items || []).length), 0) : 0,
      photos: it && it.photos ? Object.keys(it.photos).length : 0,
    }, 'build');
  } catch (err) { /* never load-bearing */ }
}
...
export async function peekBuild(id) {
  ...
  return {
    ...
    error: b.error || '',
    // Ran out of steps rather than finishing. The trip may be thin, and saying
    // so is better than letting it read as complete.
    capped: !!b.capped,
...
// How long one advance may hold a build before another may take it.
//
// A build step is one model call writing a large itinerary — T_MODEL allows two
// minutes for it. The lease has to outlast that, or a slow step gets picked up
// by a second caller while the first is still working and the trip is written
// twice.
...
export async function advanceBuild(id) {
  ...
  // Somebody else is mid-step. Say it is still building and do nothing —
  // paying twice for the same step is the failure this prevents, and it became
  // possible the moment a build could drive itself forward as well as being
  // driven by the page.
  const now = Date.now();
  if (b.lease && now - b.lease < LEASE_MS) {
    return { itinerary: b.itinerary, building: true, error: '' };
  }

  // Claim it, then read back to see whether the claim stuck.
  //
  // Checking the lease and then writing it is two operations, and two callers
  // that both read "free" both proceed — which is precisely the case this
  // exists for, since a chain handing off and a page polling can land in the
  // same instant. Firestore reads are strongly consistent, so after both have
  // written, both see the same last writer. Whoever is not it backs off.
  const mine = 'l' + now.toString(36) + Math.random().toString(36).slice(2, 8);
  await writeBuild(id, { ...b, lease: now, leaseBy: mine });
  const held = await readBuild(id);
  if (!held || (held.leaseBy && held.leaseBy !== mine)) {
    return { itinerary: b.itinerary, building: true, error: '' };
  }
  ...
  if (b.steps >= MAX_STEPS) {
    // CAPPED, not finished.
    //
    // The builder signals it is done by replying without a tool call. Reaching
    // MAX_STEPS means it never did — so it was still working, and whatever it
    // had written is what the traveller gets. Until 2026-09-06 that was
    // recorded as done with NO error at all whenever an itinerary existed, so a
    // truncated trip was indistinguishable from a complete one, to the app, to
    // the agent, and to whoever read the journal afterwards. Measured on
    // sesn_01WDtF9sMZTNcuFA9ES3nSW4: two of its three builds ended here.
    const err = b.itinerary ? '' : 'The builder ran too long.';
    logBuild(b, { capped: true, error: err });
    await writeBuild(id, { ...b, lease: 0, leaseBy: '', done: true, capped: true, error: err });
    return { itinerary: b.itinerary, building: false, capped: true, error: err };
  }
  ...
  try {
    ...
    await writeBuild(id, { ...b, lease: 0, leaseBy: '', steps: b.steps + 1 });
    return { itinerary: b.itinerary, building: true, error: '' };
  }
  ...
  if (!calls.length) {
    ...
    if (state) done = true;
      ...
      empty++;
      if (empty >= 3) {
        logBuild(b, { steps: b.steps + 1, error: 'The builder stopped answering. Try building again.' });
        await writeBuild(id, { ...b, lease: 0, steps: b.steps + 1, done: true,
  ...
  // The one place a build finishes because the builder SAID so.
  if (done) logBuild(b, { steps: b.steps + 1, itinerary: state });

  const next = {
    // Released, so the next step can start immediately rather than waiting out
    // a lease that belongs to work already finished.
    lease: 0, leaseBy: '',
