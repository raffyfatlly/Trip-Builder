...
// THE FREE TIER, in his words: "we give 7 turns without building if they don't
// pay." A turn measures 0.44 credits, so seven of them is 3.1 — rounded to 4,
// which buys nine turns and costs us at most RM0.40 per signup.
//
// The turn allowance is only half of it. The other half is that a free account
// cannot BUILD, and no credit figure can express that on its own — a build is
// 16-29 credits, so a big enough free grant would quietly buy one. See the
// build gate; this number alone is not the paywall.
export const grant = () => num('CREDIT_GRANT', 'creditGrant', 4);
