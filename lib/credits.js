...
import { readLedger, writeLedger, bumpLedger, setLedger, firestoreConfigured } from './firestore.js';
...
// RESCALED 2026-09-07, when he set the rule the whole business now runs on:
//
...
//
// RM10 of cost spread over 100 credits is ten sen each. That single number is
// what makes the ceiling true: a 100-credit pack cannot cost more than RM10 to
// serve, whatever the customer does with it, because credits ARE cost.
//
// It replaces RM0.31, which came from a measured median trip back when a trip
// was RM15.62. Moving to Haiku for the chat took a trip to RM2.73-4.83, so the
// old unit made a trip about nine credits — too coarse to show anybody.
//
// He also moved the scale himself the same day: "1000 credit seemed to much ...
// maybe around 100 instead." At ten sen a credit a trip is 27-48 credits and a
// pack is 100, which is the size of number a person can hold in their head.
export const myrPerCredit = () => num('CREDIT_COST_MYR', 'creditCostMyr', 0.10);
...
// 2.8x, and it is a FLOOR rather than a target. RM28 for 100 credits = RM28
// against at most RM10 of cost. Most customers spend nowhere near their whole
// balance — one typical trip is 44 credits — so the realised multiple is
// usually 6x or better. The 2.8 is the worst case, which is the number worth
// fixing in place.
export const markup = () => num('CREDIT_MARKUP', 'creditMarkup', 2.8);
...
// THE FREE TIER. It was 4, from "we give 7 turns without building if they don't
// pay" and a turn measured at 0.44 credits.
//
// That measurement was taken before the app grew a research pass, photo cards
// and live price checks. A real free session on 2026-09-08 — the Bali one —
// spent $0.328 across seven model calls and four messages, which is 14 credits:
// about 3.5 a turn, eight times the figure this was set from. So 4 credits was
// one turn and a bit, and raffy said what that looks like from the outside:
//
//   "4 credits is too little doesn't demonstrate a good part of the app.
//    add it to 10."
//
// 15 buys five or six turns — the onboarding, a set of photo cards, a live
// price check and a follow-up — which is enough to see what the thing actually
// does. What it costs us is exactly 15 x RM0.10 = RM1.50 per signup, and only
// if they spend every credit.
//
// It was 10 for a few hours on 2026-09-08 and he moved it again the same day,
// after watching a real free session run out mid-conversation: "for now give
// free user 15 credit." The direction of travel is the point — every time this
// number has been measured against a real session rather than a spreadsheet, it
// has gone up.
//
// THE CEILING ON THIS NUMBER IS 24, and it is not a matter of taste: a build
// costs 25, so a grant of 25 or more hands every free account a free trip and
// the paywall stops existing. The test asserts grant() < rebuildCredits().
export const grant = () => num('CREDIT_GRANT', 'creditGrant', 15);

// What the landing page tells people a rebuild costs, and therefore the only
// honest number to put in front of them before one starts.
//
// raffy, 2026-09-07, on Syahirah's Singapore trip: "the app rebuilds from
// scratch again without no telling her how much credit it will take her etc.
// so now she left with no credit."
//
// The chat agent was told not to quote a figure, because it does not know one
// and an invented number is worse than none. That was right, and it left
// nobody saying anything at all. So the app supplies the figure — the same one
// on the pricing page, which is what she would have read before she started.
// A build measures 16 credits (light) to 29 (heavy). 25 is the honest middle to
// warn somebody with before one starts — high enough that it is rarely an
// under-estimate, which is the direction that matters when the whole point is
// that nobody is surprised.
export const rebuildCredits = () => num('CREDIT_REBUILD', 'creditRebuild', 25);
...
// Nobody signed in has no account to bill. "each user must sign in with email
// address to start", so this is now a courtesy rather than an allowance: enough
// to see the app answer once and be asked to sign in, and RM0.20 if abused.
export const anonGrant = () => num('CREDIT_ANON', 'creditAnon', 2);
...
export async function ledgerFor(who, session) {
  ...
  if (got) {
    ...
    //
    // ONE FIELD, not the whole document. This used to write the ledger back
    // entire, carrying the `used` figure it had read a moment earlier — so
    // raising the grant from 4 to 10 put a stale spend total over every charge
    // that landed while it was doing it. See bumpLedger for the same bug in its
    // more expensive form.
    if (allowance > (got.granted || 0)) {
      await setLedger(id, { granted: allowance });
      return { ...got, granted: allowance };
    }
    return got;
  }
...
export async function allowed(who, session) {
  ...
  return {
    ok: left > 0,
    // Whether they have ever bought anything. The build gate turns on this
    // rather than on the balance — see the gate in lib/managedAgents.js.
    paid: l.paid === true,
    left,
    granted: l.granted || 0,
    used: l.used || 0,
 ...
 * again.
 */
export async function settle(session, actor) {
  ...
  try {
    ...
    // WHOSE credits this comes out of.
    //
    // raffy, 2026-09-07, on sharing a trip with another user: each person pays
    // for their own turns. So the charge follows whoever made THIS request, not
    // whoever owns the session — otherwise inviting four people into a trip
    // would empty the owner's balance, and a guest could spend somebody else's
    // money by typing.
    //
    // It also means a guest needs an account of their own, which is the point:
    // sharing turns one user into several rather than costing the first one
    // more. The journal's own `who` stays the OWNER, so reporting still says
    // whose trip it was.
    const payer = actor || d.who;
    ...
    //
    // Created if it is missing, then ADDED TO — never read, adjusted and
    // written back. A turn has several metered requests in flight at once and
    // they all settle; the read-modify-write this replaces lost most of the
    // charges on a busy turn to the last writer, which is how his journal came
    // to say 13 credits against a ledger that said 4.
    const l = await ledgerFor(payer, session);
    if (!l) return null;
    await bumpLedger(l.id, {
      used: d.credits,
      plan: d.plan || 0,
      build: d.build || 0,
    });
