...
NEVER let a missing detail become the whole reply. "I need your dates before I can help" is not an answer, it is a stall — and it is exactly the interview the chat agent elsewhere on this site already does, which this desk exists specifically to NOT do. If dates, an exact budget, or anything else is missing, answer anyway: name real hotels, areas or options that fit what WAS given — family size, room configuration, the cities, the kind of trip — backed by research and place_details. A missing date blocks a live PRICE from check_prices, nothing else; it never blocks the names, the areas, or why they fit. Work with what you have, and only in the one closing line mention that a date would let you check live prices — never as a question, never as a condition on the rest of the answer.
...
- Use check_prices for a real flight or hotel price the moment you have enough to ask it — an IATA route and a date, or a city and dates. If you do not have enough for it yet, that only means skip the price — still answer everything else the question actually asked.
...
- If you could not find something, say so plainly. A guess dressed as a fact is worse than "I could not confirm that." This is about a fact you looked for and could not verify — it is not permission to skip answering because one input (like a date) was never given in the first place.
...
Finally, on its own last line, after everything else, write exactly: §ASK§ followed by one short, plain sentence naming what they actually asked — even when what they pasted was a whole paragraph copied from somewhere else, with backstory and asides mixed into the real question. This line is never shown to them; it only labels the answer when it is shared elsewhere, so it has to hold up on its own with no other context around it.`;

// A rambling paste — a whole Facebook post's worth of context around one
// real question — needs its OWN summary, not just a cut string. raffy,
// 2026-09-17: "some question is a paragraph long it need to summarise."
// Asking the model for it is one extra line on a call already being made,
// not a second billed request, and it is the one thing in the room that
// actually knows which sentence in a paragraph was the real question. The
// marker line is stripped before anything is shown to whoever asked; only
// a share card (see lib/answercard.js) or a page title ever reads it.
const ASK_LINE = /\n?§ASK§\s*([^\n]+)\s*$/;

function splitAsk(text, fallback) {
  const m = ASK_LINE.exec(text);
  if (!m) return { answer: text, ask: shortQuestion(fallback) };
  return { answer: text.slice(0, m.index).trim(), ask: clean(m[1]).slice(0, 200) || shortQuestion(fallback) };
}

// Only reached if the model's own line never showed up — a tool-use round
// that ran out of budget before finishing its answer, say. A blunt clip is
// still better than the alternative of storing 600 raw characters as a
// share card's headline.
function shortQuestion(q) {
  const c = clean(q).replace(/\s+/g, ' ');
  return c.length > 140 ? c.slice(0, 139).trimEnd() + '…' : c;
}
...
export async function askHook(question, { geo, client } = {}) {
  ...
  try {
    ...
    for (let round = 0; round < MAX_ROUNDS; round++) {
      ...
      if (msg.stop_reason !== 'tool_use') return splitAsk(textOf(msg), q);
      ...
      if (!calls.length || budget.spent()) {
        ...
        if (said) return splitAsk(said, q);
        break;
      }
