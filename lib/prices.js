...
async function hotelPrices(q) {
  ...
  const head = (hotel ? hotel + ', ' : '') + where + ', ' + q.checkIn + ' to ' + q.checkOut;
  return [head,
    '  The live inventory has no rate for this one — GO AND SEARCH FOR IT YOURSELF.',
    '  You have web_search and web_fetch. Search the way a person would:',
    '    "' + (hotel || where) + ' rates ' + q.checkIn + ' to ' + q.checkOut + '"',
    '  Google shows nightly rates per room across every booking site for exactly this,',
    '  and the hotel\'s own page usually carries them too. Read the result, then fetch the',
    '  best page for the detail.',
    '',
    '  WHAT YOU OWE THE TRAVELLER when you quote what you found:',
    '  - Name the site and say when you looked. "RM406 on Booking.com just now", never a',
    '    bare number.',
    '  - Say whether each figure is PER NIGHT or for the WHOLE STAY, and never swap them.',
    '  - A site with no rooms is that site having no rooms. Do NOT say a hotel is fully',
    '    booked or sold out on the strength of a search — check another before you say it,',
    '    and if you do say it, name what you checked.',
    '  - If the search genuinely turns up nothing, say you could not find a published rate',
    '    and send them to the link. Do not estimate one, and never carry a figure off a',
    '    blog or a listicle: those are last year\'s prices for a different room.',
    '  book: ' + (link || 'no link — give me the city, not just the hotel name')].join('\n');
}
