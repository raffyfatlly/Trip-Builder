...
async function fromGoogle(search, limit) {
  ...
  // Checked here, with the key, against the real Google URL — the proxied one
  // is relative and cannot be fetched from this process.
  const live = [];
  for (const h of out) {
    if (await loads(h.absolute)) { delete h.absolute; h.checked = true; live.push(h); }
  }
  return live;
}
