...
// THE ONE RULE THAT CANNOT WAIT FOR A NEW CHAT.
//
// raffy, 2026-09-07: "how to make the agent always give me structured list view
// with photos instead of just text. cause I find I need to say explicitly give
// option to choose from then only it give me the structure list with photo."
//
// The rule is in the system prompt in full (lib/prompt.js). But a Managed
// Agents session pins the agent VERSION it was created on, so a prompt fix only
// ever reaches NEW conversations — and the trip he is complaining about is one
// he is in the middle of. This is the only channel that reaches a session
// already running: a line on each message, which the agent reads at the moment
// it decides how to answer.
//
// Both halves, because the second is what stops it papering every reply in
// cards. Deliberately short — it is paid for on every turn.
export const HOUSE_RULES =
  'House rules, every turn.'
  + ' (1) The FIRST time you name places in this conversation — hotels,'
  + ' restaurants, activities, attractions, areas — send them through `present`'
  + ' as cards with photos, not as prose or a bulleted list, and do it without'
  + ' being asked to "give options". ONE place counts — a single recommendation'
  + ' gets a card too, and it is often the strongest answer.'
  + ' Once a set is on screen, talking about those same places afterwards is'
  + ' just talk — do not re-card them.'
  // Added 2026-09-08. Same reason the cards rule is here rather than only in the
  // system prompt: it has to reach the conversations already running.
  + ' (2) Plain English. Most of these travellers do not speak it as a first'
  + ' language. Stay warm and natural, but no idioms or slang — "roughly how'
  + ' much per night", never "what is your ballpark". If a phrase does not mean'
  + ' what its words say, rewrite it.';
