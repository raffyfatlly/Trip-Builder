...
// How many free-standing questions the landing page has answered for this
// session — see lib/hook.js. A soft cap only, never shown to whoever is
// asking: raffy, 2026-09-16, "don't put the one free question thing etc."
// It exists purely so the unauthenticated endpoint that bills a real card
// (lib/config.js's apiKey()) cannot become a free API for anyone who finds
// it, not to ration a real visitor's experience.
export async function readHookAsks(session) {
  const doc = await call('/hookAsks/' + encodeURIComponent(session));
  if (!doc) return 0;
  return Number(decFields(doc.fields).n) || 0;
}

export async function writeHookAsks(session, n) {
  await call('/hookAsks/' + encodeURIComponent(session) + '?updateMask.fieldPaths=n', {
    method: 'PATCH',
    body: JSON.stringify({ fields: { n: enc(n) } }),
  });
}

// A shared answer from the landing page's ask box — see lib/answershare.js.
// Its own collection, same shape as `shares` above: a token that maps to
// content, never the other way around, so a link can be pasted anywhere
// without exposing anything about the browser that asked.
export async function readAnswerShare(token) {
  const doc = await call('/answers/' + encodeURIComponent(token));
  if (!doc) return null;
  const f = decFields(doc.fields);
  return { question: f.question || '', answer: f.answer || '', at: Number(f.at) || 0 };
}

export async function writeAnswerShare(token, question, answer) {
  const mask = ['question', 'answer', 'at'].map((x) => 'updateMask.fieldPaths=' + x).join('&');
  await call('/answers/' + encodeURIComponent(token) + '?' + mask, {
    method: 'PATCH',
    body: JSON.stringify({
      fields: {
        question: enc(String(question || '').slice(0, 600)),
        answer: enc(String(answer || '').slice(0, 4000)),
        at: enc(Date.now()),
      },
    }),
  });
...
export async function bumpLedger(id, deltas) {
  ...
  // THE ID GOES IN RAW. A Firestore resource name is not a URL — it is the
  // document's actual path, and percent-encoding it does not address the same
  // document, it names a DIFFERENT one.
  //
  // This shipped encoded, and within an hour it had split raffy's test account
  // in two: `u:tes@user.com` held the balance the app reads, while every charge
  // landed in a second document called `u%3Ates%40user.com`. The app showed
  // "0 credits" under every reply because the balance it reads never moved.
  //
  // It passed a live check against the real database, which is the part worth
  // remembering: the check used the id `zz-atomic-check-<timestamp>`, and an id
  // with no ':' or '@' in it cannot show this bug. Test with a real-shaped key.
...
// --- errands ---------------------------------------------------------------
//
// One document, holding a short queue of things the deployment should do on my
// behalf and the answers it got. See lib/chores.js for why this exists at all.
// Stored as JSON in a single field for the same reason the config document is:
// no schema to keep in step, and one read.

export async function readChores() {
  const doc = await call('/config/chores');
  if (!doc) return null;
  try { return JSON.parse(decFields(doc.fields).values || 'null'); } catch (e) { return null; }
}

export async function writeChores(next) {
  await call('/config/chores?updateMask.fieldPaths=values', {
    method: 'PATCH',
    body: JSON.stringify({ fields: { values: enc(JSON.stringify(next || {})) } }),
  });
  return next;
}
