...
import { dateChangeKind } from './dates.js';
  ...
  for (const call of pending) {
   ...
   try {
    ...
    if (currentItinerary && !(call.input && call.input.confirmed === true)) {
      ...
      // Is this only a date change that keeps the trip the same length? Then
      // it is not a rebuild at all — shift_dates does it for nothing, and
      // saying so is more useful than asking her to approve a cost she should
      // never have been offered. This is exactly Syahirah's case.
      const kind = dateChangeKind(currentItinerary, input.start, input.end);
      // "Singapore" against a trip titled "Singapore" — either string may be
      // the longer one ("Singapore" vs "Singapore, Singapore"), so check both
      // ways round. When it cannot tell, it falls through to the ordinary
      // confirmation, which is the safe direction.
      const norm = (x) => String(x || '').trim().toLowerCase();
      const a = norm(input.destination);
      const b = norm((currentItinerary.trip || {}).title);
      const sameCity = !!a && !!b && (a.includes(b) || b.includes(a));
      // A date change in the same place is never a rebuild any more, whether the
      // trip keeps its length or not: the same length is shift_dates, and a
      // different length is that plus add_day / remove_day. Both are free, so
      // offering to charge for one instead would be taking money for nothing.
      const dateOnly = sameCity && (kind === 'shift' || kind === 'rebuild');
      const text = dateOnly
        ? ['DO NOT REBUILD — this is a date change in the same place, and every part of it is a free edit.',
           kind === 'shift'
           ...
           'This costs them nothing and takes no time. Do it now rather than asking.'].join('\n')
        : ['NOT STARTED — a rebuild needs their yes first, and this call did not have it.',
           ...
           '  a hotel swap, renaming it, anything on a day — all of those are free edits. If this is',
           '  one of those, do that instead and do not ask them to pay for it.',
