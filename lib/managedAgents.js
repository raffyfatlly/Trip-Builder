...
//
// Cumulative by nature — the log holds every request the session ever made —
// so this is written as a TOTAL rather than added. Anything else would count
// the same tokens once per poll, and this runs every few seconds.
function chatSpend(events) {
  const u = { in: 0, out: 0, cacheRead: 0, cacheWrite: 0, calls: 0 };
  let model = '';
  for (const e of events || []) {
    const x = (e && e.usage) || (e && e.type === 'session.usage' ? e : null);
    if (!x || !(x.input_tokens || x.output_tokens)) continue;
    u.calls++;
...
export async function advanceState(chatSessionId, ms = 240000) {
  ...
  try {
    const { u, model } = chatSpend(await listEvents(chatSessionId));
    if (u.calls) spendTotal(chatSessionId, 'chat', model || 'claude-sonnet-5', u);
  ...
  try {
    ...
    if (after.itinerary && !after.building) {
      ...
      if (Object.keys(next).length !== Object.keys(known).length) {
