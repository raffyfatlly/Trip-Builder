  ...
  for (const call of pending) {
    ...
    if (call.name === 'research') {
      ...
      try {
        ...
        text = 'The research desk failed: ' + err.message
      ...
      if (out && out.usage) {
        ...
        spendAdd(chatSessionId, 'research', out.model, {
...
function stepDetail(name, input) {
  ...
  if (name === 'research') {
    const qs = (i.questions || []).map((q) => cut(q, 64)).filter(Boolean);
    return qs.join('\n');
  }
...
function actionOf(e) {
  ...
  switch (e.name) {
    ...
    case 'research': {
      const n = (i.questions || []).length;
      return n ? { icon: 'search', text: 'Looked up ' + n + (n === 1 ? ' thing' : ' things'),
        detail: (i.questions || []).map((q) => String(q).trim()).join('\n') } : null;
    }
