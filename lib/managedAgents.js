...
import { orBuilderReady, startBuild, advanceBuild } from './orBuilder.js';
...
export async function getState(chatSessionId) {
  ...
  const resolve = async () => {
    ...
    if (latest.kind === 'openrouter') {
      // One model call per poll, same bounded-work rule as the session path.
      const r = await advanceBuild(latest.id);
      return { itinerary: r.itinerary, building: r.building, builder: latest.id, buildError: r.error };
    }

    const { events: bEvents, answered } = await pumpBuilder(latest.id);
    return {
